...
const SearchButton: React.FC<SearchButtonProps> = ({ setIsSearchFilterShown, }) => {
  const location = useSelector<LocationReduxCombinerState, LocationReduxCombinerState["location"]["name"]> ((state) => state.location.name);
  const totalCount = useSelector<CountReduxCombinerState, CountReduxCombinerState["guestCount"]["totalCount"]> ((state) => state.guestCount.totalCount);
