...
const incrementValue = (count: number): number => {
  return (count += 1);
}

const decrementValue = (count: number): number => {
  return (count -= 1);
}

export const guestCountReducer = ( state: CountState = initialState, action: CountType ) => {
  switch (action.type) {
